...
if FEC
    data_coded = LoRa_Hamming_enc(data_tx,4);
    data_sym_coded = bi2de(reshape(data_coded,SF,[]).','left-msb');
    data_sym = data_sym_coded;
end

tx = [];
% ИСПРАВЛЕНИЕ: добавлено транспонирование (.') к data_sym
for m = data_sym.' 
...
for n = 1:cfg.Nsym
    ...
    decoded_bits = [decoded_bits LoRa_Hamming_dec(rx_bits,4)];
...
if isequal(decoded_bits,data_tx)
    disp(100000)
end