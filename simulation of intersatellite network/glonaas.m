...
r_orbits = [ 25500000];  % примерные орбитальные радиусы (м)
...
numPerOrbit = 145;  % число спутников на каждой орбите
...
for m=1:3%m=1:round(2*pi*6560000/1500000)
    inclination_inc = deg2rad(60);
    RAAN_inc = deg2rad(120*m); %RAAN_inc = deg2rad(rad2deg(acos((-(1500)^2+2*(6560)^2)/(2*(6560)^2)))*m);
    phaseShift_inc = 0;
    for idx = 1:length(r_orbits)
        r = r_orbits(idx);
        h = r - R_earth;
...
if best_i == 0 || best_j == 0
    ...
    fprintf('\nПара узлов с максимальным физическим расстоянием и существующим маршрутом: %d и %d (%.2f км)\n', best_i, best_j, maxD/1000);
end




%% === Поиск маршрута с накоплением задержки через сеть (с учетом матрицы W) ===
...
% ---------------------------------------------------------длина антенны площадь сферы, сигнал шум => дальность--
...
%% оценка через калькуляцию среднего снр
